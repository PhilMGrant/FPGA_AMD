...
static void lz4CompressPart2(hls::stream<uint8_t>& in_lit_inStream,
    ...
    // 优化：流水线寄存器
    ap_uint<32> lit_len_tmp;
    ap_uint<16> match_len_tmp;
    ap_uint<16> match_off_tmp;
    bool is_special_end = false;
    bool is_normal_end = false;
    bool lit_len_ge_15 = false;
    bool lit_len_gt_0 = false;
    bool match_len_ge_15 = false;
    bool match_len_ge_255 = false;
    bool lit_len_ge_255 = false;

    // 优化：添加更多流水线阶段
    ap_uint<8> outValue_reg = 0;
    bool should_write_reg = false;
    bool readOffsetFlag_next = true;
...
#pragma HLS LOOP_FLATTEN off
        ...
        if (readOffsetFlag) {
            ...
            // 优化：并行预计算所有条件判断
            is_special_end = (match_len_tmp == 777) && (match_off_tmp == 777);
            is_normal_end = (match_off_tmp == 0) && (match_len_tmp == 0);
            lit_len_ge_15 = (lit_len_tmp >= 15);
            lit_len_gt_0 = (lit_len_tmp != 0);
            match_len_ge_15 = (match_len_tmp >= 15);
            
            // 预计算 offset+1
            match_offset_plus_one = match_off_tmp + 1;
        } else {
            readOffsetFlag_next = readOffsetFlag;
        }
        ...
        switch (next_state) {
            case WRITE_TOKEN: {
                ...
                // 优化：并行计算输出值 - 使用查找表优化
                ap_uint<4> lit_token = 0;
                ap_uint<4> match_token = 0;
                
                // 优化：使用条件赋值而不是三元运算符
                if (lit_len_ge_15) {
                    lit_token = 15;
                } else if (lit_len_gt_0) {
                    lit_token = lit_length;
                }
                
                if (match_len_ge_15) {
                    match_token = 15;
                } else {
                    match_token = match_length;
                }
                
                outValue.range(7, 4) = lit_token;
                outValue.range(3, 0) = match_token;
                
                // 优化：并行状态转移逻辑 - 使用寄存器存储状态
                enum lz4CompressStates next_state_tmp = WRITE_TOKEN;
                if (lit_len_ge_15) {
                    lit_length -= 15;
                    next_state_tmp = WRITE_LIT_LEN;
                } else if (lit_len_gt_0) {
                    lit_length = 0;
                    next_state_tmp = WRITE_LITERAL;
                } else {
                    next_state_tmp = WRITE_OFFSET0;
                }
            ...
            case WRITE_MATCH_LEN: {
                match_len_ge_255 = (match_length >= 255);
