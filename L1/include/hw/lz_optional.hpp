...
dtree_init:
    ...
    lz77_divide:
        while (true) {
#pragma HLS LOOP_TRIPCOUNT min = 1048576 max = 1048576
#pragma HLS PIPELINE II = 2  // 增加II以改善时序
#pragma HLS dependence variable = lcl_dyn_ltree inter false
#pragma HLS dependence variable = lcl_dyn_dtree inter false
            uint32_t ltreeIdx, dtreeIdx;
            ...
            if (is_literal_state) {
                ...
                // 优化：预计算ltreeIdx和dtreeIdx
                bool has_match = (tLen != 0);
                ltreeIdx = has_match ? (length_code[tLen] + 256 + 1) : tCh;
                dtreeIdx = has_match ? d_code(tOffset, dist_code) : 0;
            ...
            } else if (is_offset0_state) {
                outValue.data[0] = tOffset1;
                next_state = WRITE_OFFSET1;
            } else if (is_offset1_state) {
...
#pragma HLS PIPELINE II = 1  // 从底层逻辑优化，尝试恢复II=1
...
#pragma HLS dependence variable = local_mem inter false
            ...
            // 优化：使用状态机寄存器减少组合逻辑
            static ap_uint<1> matchFlag_reg = 0;
...
#pragma HLS BIND_STORAGE variable = local_mem type = ram_2p impl = bram
    ...
    static ap_uint<1> matchFlag_reg = 0;
    ap_uint<1> outFlag = 0;
    ap_uint<1> boostFlag = 0;
    static ap_uint<16> skip_len_reg = 0;
    
    // 优化：使用静态寄存器避免phi操作
    static uint8_t nextMatchCh = local_mem[match_loc_reg % BOOSTER_OFFSET_WINDOW];

    lz_booster:
        for (uint32_t i = 0; i < (input_size - LEFT_BYTES); i++) {
#pragma HLS PIPELINE II = 3  // 进一步增加II值以彻底解决时序问题
#pragma HLS dependence variable = local_mem inter false
        // 阶段1: 数据读取和local_mem写入
        compressd_dt inValue = inStream.read();
        uint8_t tCh = inValue.range(7, 0);
        uint8_t tLen = inValue.range(15, 8);
        uint16_t tOffset = inValue.range(31, 16);
        
        // 优化：并行写入local_mem，减少关键路径
        local_mem[i % BOOSTER_OFFSET_WINDOW] = tCh;
        
        // 阶段2: 条件计算和状态更新
        boostFlag = (tOffset < BOOSTER_OFFSET_WINDOW);
        outFlag = 0;
        ...
        // 阶段3: 寄存器更新和下一字符准备
        // 优化：减少local_mem访问次数，避免复杂的phi操作
        matchFlag_reg = new_matchFlag;
        skip_len_reg = new_skip_len;
        match_len_reg = new_match_len;
        match_loc_reg = new_match_loc;
        
        // 优化：预读取下一字符，使用静态寄存器避免phi操作
        // 使用静态寄存器直接更新，避免复杂的多路选择器
